...
public class BDDFileParser {
    private List<String> allBackgroundTypes;


    public BDDFileParser() {
        allBackgroundTypes = new ArrayList<>();
        for(SnippetType type: SnippetType.values()) {
            allBackgroundTypes.add(String.format("background(%s):", SnippetType.getName(type)));
        }
    }
    ...
    private boolean isBackgroundLine(String line) {
        List<String> list = allBackgroundTypes;
        for (String s : list) {
    ...
    private void parseTests(BDDFeature feature, List<String> lines) {
        ...
        for (int index = feature.startLineIndex; index < lines.size(); index++) {
            ...
            if (line.startsWith("---")) {
                ...
                currentTest.skip = b;
            } else if (currentTest != null && line.startsWith("useConverterFromTest:")) {
                Integer testIndex = Integer.parseInt(parseArg(line));
                currentTest.useConverterFromTest = testIndex;
