...
//	public boolean compareObj(ThenValue thenVal, DValue dval, DeliaLog log) {
//		if (thenVal.isEmpty()) {
...
//	}
	public boolean compareMultiObj(ThenValue thenVal, List<String> actualL, FieldCopyLog log) {
		if (thenVal.isEmpty()) {
			String err = String.format("value-mismatch: struct must use then: clause");
			log.logError(err);
			return false;
	...
	private boolean compareStringLists(List<String> genL, List<String> thenL, FieldCopyLog log) {
...
//		return helper.adjustForDBType(thenL);
	...
	public boolean compareError(ThenValue thenVal, List<FCError> errors, FieldCopyLog log) {
		List<String> thenL = generateFromThen(thenVal);
